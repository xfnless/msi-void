...
- 布局和标签变化安静等待下次保存，不显示未保存警报
- 未保存离开页面时使用浏览器原生提醒
- 一个主密码同时派生本地解密密钥和独立的后端认证凭证
- 已解锁时可以原子修改主密码，不重新加密全部条目
- PBKDF2-SHA-256 + AES-256-GCM 客户端加密；服务器收不到主密码
...
打开 `http://127.0.0.1:8080`。首次进入时创建主密码。浏览器从同一个主密码分别派生数据解锁密钥和后端认证凭证；后端只收到派生凭证，永远收不到主密码或数据密钥。部署到公网时不要使用 `-secure-cookie=false`。

新主密码至少 16 个字符，推荐使用 4–6 个互不相关的随机单词。认证使用 PBKDF2-HMAC-SHA256 600,000 次迭代和独立随机盐，连续登录失败会触发指数退避。修改密码只重新包裹随机数据密钥并轮换后端认证哈希；SQLite 事务保证两部分同时生效。
...
输出文件必须不存在，避免误覆盖。备份仍是密文数据库，恢复时必须保留原主密码。恢复只需停服务，把选定备份作为 `-database` 指向的文件，再启动服务。建议把多个日期的数据库复制到至少两个独立位置，并定期实际演练解锁。
...
SQLite 中只有加密对象、认证凭证哈希、版本号、对象类型、更新时间及加密参数。条目正文、主密码、搜索词、标签查询、工作区布局和密钥明文不会写入 URL、localStorage、IndexedDB 或服务器日志。对象类型目前预留 `entry`、`workspace`、`query`、`view`；以后可在保持相同“对象 → 查询 → 视图”边界的前提下加入 TSV、附件引用和专用查看器。
